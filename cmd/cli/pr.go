...
Requires:
  ...
  - Target workload annotated with gerty.io/repo and gerty.io/path
  - gerty-server running with recommendations available

Use --dry-run to preview changes without creating a PR.`,
		Example: `  gerty pr deployment/api-gateway
  gerty pr deployment/api-gateway -n production
  gerty pr statefulset/redis --dry-run
  gerty pr deployment/api-gateway --branch-prefix gerty-resize`,
	...
	cmd.Flags().StringVar(&branchPrefix, "branch-prefix", "gerty-rightsizing", "Branch name prefix")
