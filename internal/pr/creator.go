...
// Annotations holds gerty GitOps annotations from a workload.
...
func (p *PRCreator) Create(ctx context.Context, opts Options) (*PRResult, error) {
	...
	tmpDir, err := os.MkdirTemp("", "gerty-pr-*")
	if err != nil {
...
// readAnnotations reads gerty.io/* annotations from the target workload.
func (p *PRCreator) readAnnotations(ctx context.Context, kind, name, namespace string) (*Annotations, error) {
	...
	repo := raw["gerty.io/repo"]
	path := raw["gerty.io/path"]
	if repo == "" || path == "" {
		return nil, fmt.Errorf("missing required annotations gerty.io/repo and gerty.io/path on %s", target)
	}

	return &Annotations{
		Repo:  repo,
		Path:  path,
		Field: raw["gerty.io/field"],
	}, nil
}
...
func FormatPRBody(rec *models.Recommendation, kind, name, namespace string) string {
	...
	b.WriteString("\n---\nGenerated by [gerty](https://github.com/gerty-labs/gerty)\n")
